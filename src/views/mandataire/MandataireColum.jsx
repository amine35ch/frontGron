...
const RowOptions = ({ row, resource, resource_name }) => {
  ...
  const disabledMandataireValidation = async event => {
  ...
  const deletedMandataireValidation = async event => {
  ...
  return (
    <>
      ...
      <DialogAlert
        ...
        title={`${row?.state == '0' ? 'Activer' : 'Désactiver'} Mandataire ${row?.reference} ?`}
        acceptButtonTitle='Accepter'
        declineButtonTitle='Annuler'
        handleAction={disabledMandataireValidation}
      />
      <DialogAlert
        open={dialogDeleteOpen}
        description=''
        setOpen={setDialogDeleteOpen}
        title={`Voulez-vous supprimer Mandataire ${row?.reference} ?`}
        acceptButtonTitle='Accepter'
        declineButtonTitle='Annuler'
        handleAction={deletedMandataireValidation}
      />
    </>
  )
}

const MandataireColum = ({ userRole, resource, resource_name }) => {
  const auth = useAuth()
          ...
          renderCell: ({ row }) => (
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              {listPermissions?.permissions?.find(item => item.name == `view ${resource}`)?.authorized && (
                <Tooltip title='Voir'>
                  <IconButton size='small' component={Link} sx={{ mr: 0.5 }} href={`/agents/${row.id}/details`}>
                    <Icon icon='mdi:eye-outline' fontSize='20px' />
                  </IconButton>
                </Tooltip>
              )}

              {row?.can_update === false && row?.can_delete === false ? null : (
...
export default MandataireColum
